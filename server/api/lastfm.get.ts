...
const getLastFmData = defineCachedFunction(
  async (username: string, apiKey: string): Promise<MusicResponse> => {
    ...
    const recentTracks: MusicTrack[] = recent.recenttracks.track.map(
      (track) => ({
        name: track.name,
